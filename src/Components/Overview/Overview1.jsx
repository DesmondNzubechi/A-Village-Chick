...
<h1 className="text-slate-900 capitalize md:hidden md:text-[45px] leading-[45px] md:leading-[50px] text-[40px]   font-poppins">{theOverView?.newsHeadline}</h1>
  ...
  <div data-aos='fade-up' data-aos-duration='1000' className="flex-start flex flex-col gap-4">
    <h1 className="text-slate-900 hidden md:flex md:text-[45px] leading-[45px] md:leading-[50px] text-[40px] capitalize  font-poppins">{theOverView?.newsHeadline}</h1>
  ...
  <Link to={`/blog/${theOverView?.headline}`} className="text-start bg-slate-900 text-slate-50 text-[30px] rounded-[1px] hover:bg-slate-800 w-fit p-3">Read more...</Link>
  </div>
</div>
        </Link>
    )

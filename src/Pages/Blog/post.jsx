...
export const BlogPost = () => {
    const {readMoreClicked, Subscribe, /*setFullNews, fullNews*/ article} = useContext(Context);
  
    return(
        <div className="grid grid-cols-1 py-[70px] px-[40px] md:grid-cols-2 gap-[100px]">
        {BlogContent.map((post) => {
          return (
            <div key={post.headline} className="flex items-start flex-col gap-[20px]">
              <Link to={`/News/${post.headline}`}>
                <img src={post.newsImg} alt="" />
              </Link>
              ...
              <Link
                to={`/News/${article?.headline}`}
                onClick={() => Subscribe(post)} // Pass a function reference here
                className="text-[20px] font-poppins bg-slate-900 p-1 text-slate-50 rounded-[1px] hover:bg-slate-800"
              >
                Read More
              </Link></div>
        );
